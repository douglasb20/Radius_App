...
$.fn.dataTable.Api.register('btnsAdicionais', function (data, index) {
  ...
  this.buttons(1, null).container().appendTo($('.dt-buttons.btn-group .btn-group:eq(0)'));
});
  ...
  return this.api()
    ...
    .map(function (td, i) {
      ...
      if (/<[a-z][\s\S]*>/i.test(data)) {
        // Não é botão e nem input, pego o texto dentro caso for html
        data = $(data).text();
      }
...
function atualizaCampoDeDataTable(input_on_change, campo, tabela, draw = true) {
  var row = $('#' + tabela)
    .DataTable()
    .row($(input_on_change).parents('tr'));
  var data = row.data();
  var index = row.index();
  data[campo] = input_on_change.value;
  $('#' + tabela)
    .dataTable()
    .fnUpdate(data, index, undefined, false);
...
function renderCpfCnpj(data, type, row) {
  if (data.length == 11) {
    ...
    return `${data.slice(0, 2)}.${data.slice(2, 5)}.${data.slice(5, 8)}/${data.slice(
      8,
      12,
    )}-${data.slice(12, 14)}`;
  }
...
function geraLinhaNoDropdown(botao) {
  ...
  return (
    ...
    botao.icone +
    ` position-absolute start-0 ps-2 ms-1 mt-1"></i>` +
    botao.texto +
    `</button></li>`
  );
}
